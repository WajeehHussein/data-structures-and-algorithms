...
function transformToLis(obj) {
  let arr = []
  Object.entries(obj).map(([key, value]) => arr.push(`<li>${key}: ${value}</li>`));
  return arr
...
const count = (target, input) => {
  let count = 0
  input.map(e => {
    e.filter(e => (e === target) ? count++ : count)
  })
  return count
};
...
const totalSum = (input) => {
  let result = input.reduce((a, b) => a.concat(b))
...
let findMaleAndFemale = (data) => {
  let result = [];
  data.map(e => { e.gender === 'male' || e.gender === 'female' ? result.push(e.name) : 0; });
...
let findShortest = (data) => {
  let low = '900';
  data.map(e => e.height > low ? low = e.name : '');
  return low;
